...
\pcalshadingtrue
\csyntaxtrue
\@x{ {\p@mmalgorithm} DieHarder}%
 \@x{\@s{8.2} {\p@lbrace} {\p@variable} injug \.{=} [ j \.{\in} Jugs
 \.{\mapsto} 0 ] {\p@semicolon}}%
\@x{\@s{17.78} {\p@lbrace} {\p@while} {\p@lparen} {\TRUE} {\p@rparen}}%
 \@x{\@s{27.36} {\p@lbrace} {\p@either} {\p@with} {\p@lparen} j \.{\in} Jugs
 {\p@rparen}}%
\@y{\@s{0}%
 fill \ensuremath{Jug[j]
}}%
\@xx{}%
\@x{\@s{70.11} {\p@lbrace} injug [ j ] \.{:=} Capacity [ j ] {\p@rbrace}}%
 \@x{\@s{36.95} {\p@or}\@s{18.84} {\p@with} {\p@lparen} j \.{\in} Jugs
 {\p@rparen}}%
\@y{\@s{0}%
 empty \ensuremath{Jug[j]
}}%
\@xx{}%
\@x{\@s{70.11} {\p@lbrace} injug [ j ] \.{:=} 0 {\p@rbrace}}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 When pouring from \ensuremath{j} to \ensuremath{k}, we first ask if
 \ensuremath{In[j] \.{+} In[k] \.{>} In[k]}. If
 so, we make \ensuremath{In[k]\.{'} \.{=} Cap[k]} and \ensuremath{In[j]\.{'}
 \.{=} In[j] \.{-} (Cap[k] \.{-} In[k]}); if
 not, we make \ensuremath{In[k]\.{'} \.{=} In[k] \.{+} In[j]} and
 \ensuremath{In[j]\.{'} \.{=} 0}. In the former case,
 the amount poured is \ensuremath{Cap[k] \.{-} In[k]}; in the latter case,
 the amount
 poured is \ensuremath{In[j]}, which equals (\ensuremath{In[k] \.{+} In[j])
 \.{-} In[k]}. In both cases,
 the amount poured is \ensuremath{Min(In[k] \.{+} In[j],\, In[k]) \.{-}
 In[k]}.
\end{cpar}%
\end{lcom}%
 \@x{\@s{32.8} {\p@or}\@s{16.4} {\p@with} {\p@lparen} j \.{\in} Jugs ,\, k
 \.{\in} Jugs \.{\,\backslash\,} \{ j \} {\p@rparen}}%
\@y{\@s{0}%
 pour from \ensuremath{j} to \ensuremath{k
}}%
\@xx{}%
 \@x{\@s{63.51} {\p@lbrace} {\p@with} {\p@lparen} poured \.{=} Min ( injug [ j
 ] \.{+} injug [ k ] ,\, Capacity [ k ] )}%
\@x{\@s{149.95} \.{-} injug [ k ] {\p@rparen}}%
 \@x{\@s{73.10} {\p@lbrace}\@s{4.1} injug [ j ]\@s{1.16} \.{:=} injug [ j
 ]\@s{1.16} \.{-} poured}%
 \@x{\@s{73.10} \.{\p@barbar}\@s{4.79} injug [ k ] \.{:=} injug [ k ] \.{+}
 poured}%
 \@x{\@s{12.29} {\p@rbrace} {\p@rbrace} {\p@rbrace}\@s{24.59} {\p@rbrace}
 {\p@rbrace}}%
\@y{}%
\@xx{}%
...
\@xx{}%
\@pvspace{8.0pt}%
\@x{ NecessaryCondition \.{\defeq} \A\, j \.{\in} Jugs \.{:}}%
 \@x{\@s{8.2} Divides ( SetGCD ( \{ Capacity [ k ] \.{:} k \.{\in} Jugs \} )
 ,\,}%
\@x{\@s{44.99} injug [ j ] )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Proof by induction that existence of a solution implies that the
 \ensuremath{GCD} of
...
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Every pouring event consists in either filling the destination jug
 \ensuremath{k} or
 in emptying the source jug \ensuremath{j} into \ensuremath{k}.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 A sequence of pouring events must begin with \ensuremath{j} full and the
 \ensuremath{k} empty
 ...
 or empty, with at least one jug empty. That means \ensuremath{injug[j]} is
 either
 \ensuremath{Capacity[j]} or 0 for every \ensuremath{j}. The invariant is
 maintained because the
 \ensuremath{GCD} of all jugs\mbox{'} capacities divides every jug capacity
 individuallyk,
 and the \ensuremath{GCD} of all jugs\mbox{'} capacities divides 0.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 In case the capacity of the destination jug \ensuremath{k} is greater than or
 equal
 to the contents of the source jug \ensuremath{j}, the entire contents of
 \ensuremath{j} is poured
 into \ensuremath{k}. In this case, because the amount poured is equal to
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{Capacity[j]}, and because the \ensuremath{GCD} of all
 jugs\mbox{'} capacities divides the
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{Capacity[j]} (the \ensuremath{GCD} of all jugs\mbox{'} capacities
 divides every individual
 jug capacity), the invariant is maintained.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 In case the capacity of \ensuremath{k} is less than the contents of
 \ensuremath{j}, then \ensuremath{k} will
 be filled and \ensuremath{injug[j]} will be reduced by
 \ensuremath{Capacity[k]}, that is,
 \ensuremath{injug[j]} will become \ensuremath{Capacity[j] \.{-}
 Capacity[k]}. But the \ensuremath{GCD} of all
 jugs\mbox{'} capacities divides this quantity, so the invariant is
 maintained.
\end{cpar}%
\vshade{5.0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
Now assume an intermediate situation where the jugs have various
 amounts in them, \ensuremath{injug[j]} for all \ensuremath{j} in
 \ensuremath{Jugs}, where all \ensuremath{injug[j]} are
 divisible by the \ensuremath{GCD}, by the induction hypothesis. A pouring
 event
 will replqce \ensuremath{injug[j]} with \ensuremath{injug[j] \.{-}
 amount\_poured} and \ensuremath{injug[k]} with
 \ensuremath{injug[k] \.{+} amount\_poured}, where
 \ensuremath{amount\_poured} is either \ensuremath{injug[j]},
 emptying \ensuremath{j}, or \ensuremath{Capacity[k] \.{-} injug[k]}, either
 emptying \ensuremath{j} or leaving
 some in it. But all quantities are divisible by \ensuremath{GCD} of all
 jugs\mbox{'}
...
\begin{cpar}{0}{F}{F}{0}{0}{}%
 ...
 \ensuremath{Feb} 17 09:13:53 \ensuremath{PST} 2014 by \ensuremath{bbeckman
}%
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 \ensuremath{\.{\,\backslash\,}}* Created \ensuremath{Mon} \ensuremath{Feb} 17
 07:51:51 \ensuremath{PST} 2014 by \ensuremath{bbeckman
}%
\end{cpar}%
\end{lcom}%
\end{document}
